        ...
        <?php $no=(int)1;foreach ($loan as $rows): $status=$rows->status;?>
          <tr>
            ...
            <td>
              ...
              <?php elseif($status==(int)2): ?>
                <?php echo anchor('loan_in/view/'.$rows->kd_pinj,'<i class="fa fa-search"></i>',array('class'=>'btn btn-info btn-xs','target'=>'_blank','data-toggle'=>'tooltip','title'=>' SURAT PERMOHONAN')); ?>
                <?php echo anchor('loan_in/receipt/'.$rows->kd_pinj,'<i class="fa fa-print"></i>',array('class'=>'btn btn-success btn-xs','target'=>'_blank','data-toggle'=>'tooltip','title'=>'SURAT PERSETUJUAN')); ?>
                <?php echo anchor('loan_in/invoice/'.$rows->kd_pinj,'<i class="fa fa-newspaper-o"></i>',array('class'=>'btn btn-warning btn-xs','target'=>'_blank','data-toggle'=>'tooltip','title'=>'BUKTI PENGELUARAN KAS')); ?>
              <?php else: ?>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div id="jml_pinj-group" class="form-group">
          ...
          <input type="text" id="rp1" name="jml_pinj" class="form-control input-sm" placeholder="0" onkeypress="return blockNonNumbers(this,event,true,false)" />
        </div>
        <div id="tenor-group" class="form-group">
          <label class="control-label">LAMA:</label>
          <input min="0" type="text" name="tenor" id="tenor" class="form-control input-sm" placeholder="0" onkeypress="return blockNonNumbers(this,event,true,false)" onkeyup="return show_payment()"/>
        </div>
        <div id="bunga-group" class="form-group">
          <label class="control-label">BUNGA FLAT</label>
          <input type="number" value="0.06" min="0.00001" name="bunga" id="bunga" class="form-control input-sm" placeholder="0" onkeypress="blockNonNumbers(this,event,true,false)" onkeyup="return show_payment()"/>
        </div>
        <div class="form-group">
          <label class="control-label">ANGSURAN PERBULAN:</label>
          <input type="text" name="jml_angs" id="perbulan" class="form-control input-sm" placeholder="0" readonly="readonly"/>
        </div>
        ...
        <?php echo form_hidden('pinj_ke',(int)1); ?>
        <?php echo form_close(); ?>
      </div>
...
<script type="text/javascript">
  ...
  function show_payment() {
    ...
    if ((document.calc.jml_pinj.value == null || document.calc.jml_pinj.value.length == 0)){ 
      ...
      var amount= document.getElementById('rp1').value.replace(/,/g,"");
      var lama = document.getElementById('tenor').value;
      var bunga = document.getElementById('bunga').value.replace(/,/g,".");
      var bunga_persen = bunga/100;
      var angsuran = rupiah(amount * bunga_persen / (1-(Math.pow(1/(1+bunga_persen),lama))));
      if(typeof angsuran != 'undefined'){ 
        document.getElementById('perbulan').value = angsuran;
      }
    }
  }
  function add() {
    ...
    $('.text-loan').text('TAMBAH DATA PINJAMAN KOPERASI');
  ...
  function edit(kd_pinj) {
    ...
    $.ajax({
      url : '<?php echo site_url('loan_in/get_by_id/')?>',
      type: 'POST',
      ...
      success: function(data){
        console.log(data);
        $('#modal-loan').modal('show');
        $('.text-loan').text('UBAH DATA PINJAMAN KOPERASI');
  ...
  function save() {
    ...
    $('.help-block').remove();
    $.ajax({
      url : '<?php echo site_url('loan_in/save/') ?>',
      type: 'POST',
      data: $('#form').serialize(),
      dataType: 'JSON',
      success: function(data){
  ...
  function update() {
    ...
    $('.help-block').remove();
    $.ajax({
      url : '<?php echo site_url('loan_in/update/') ?>',
      type: 'POST',
      data: $('#form').serialize(),
      dataType: 'JSON',
      success: function(data){
